...
import { canHaveChildren } from "@/lib/treeUtils";

const addableTypes: Array<Exclude<ElementType, "section">> = ["div", "heading", "paragraph", "button", "image"];
...
function TreeNode({
  ...
  onQuickAdd: (type: Exclude<ElementType, "section">, targetId: string) => void;
  depth?: number;
}) {
...
export function ElementTree({
  ...
  onSelect: (id: string) => void;
  onAddElement: (type: Exclude<ElementType, "section">, targetId?: string) => void;
}) {
      ...
      <section className="mb-4 rounded-2xl border border-slate-800 bg-slate-900/70 p-3">
        ...
        <div className="grid grid-cols-2 gap-2">
          {addableTypes.map((type) => (
            <button key={type} type="button" onClick={() => onAddElement(type)} className="rounded-lg border border-slate-800 px-3 py-2 text-left text-xs font-semibold text-slate-300 transition hover:border-cyan-300 hover:bg-slate-800 hover:text-white">
              {type === "image" ? "Image placeholder" : type[0].toUpperCase() + type.slice(1)}
            </button>
          ))}
        </div>
      </section>
