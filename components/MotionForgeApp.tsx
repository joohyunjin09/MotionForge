...
import type { AnimationConfig, ElementNode, ElementType, StyleConfig, Viewport } from "@/lib/types";
import {
  addNodeToTree,
  cloneTree,
  createDefaultNode,
...
export function MotionForgeApp() {
  ...
  const applyTreeMutation = (mutation: { tree: ElementNode; selectedId: string }) => {
    setTree(mutation.tree);
    setSelectedId(mutation.selectedId);
  };

  const handleAddElement = (type: Exclude<ElementType, "section">, targetId = selectedId) => {
